...
const searchRegex = /if \(\['paid', 'completed', 'served'\]\.includes\(full\.status\)\) \{[\s\S]*?store\.journal_entries = \[\.\.\.entries, \.\.\.\(store\.journal_entries \|\| \[\]\)\];\n      \}/;

const replacement = `if (['paid', 'completed', 'served'].includes(full.status)) {
